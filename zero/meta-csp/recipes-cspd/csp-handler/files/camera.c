...
static void jpeg_writefile(uint8_t *prgb, int width, int height)
{
...
static int capture_jpeg_frame(const char *camera_dev)
{
	...
	t = ((double)usage.ru_utime.tv_sec * 1e+3 + (double)usage.ru_utime.tv_usec * 1e-3);

	yuyv_to_rgb(yuyvbuf, rgbbuf, length);

	getrusage(RUSAGE_SELF, &usage);
	t = ((double)usage.ru_utime.tv_sec * 1e+3 + (double)usage.ru_utime.tv_usec * 1e-3) - t;
	printf("convert time: %3.3f msec\n", t);

	jpeg_writefile(rgbbuf, width, height);
